...
class BoxesList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (store.boxes.isEmpty) {
      return EmptyBoxesView(
        onAddBoxPressed: () => BoxEditor.show(context),
      );
    }
...
class EmptyBoxesView extends StatelessWidget {
  const EmptyBoxesView({super.key, required this.onAddBoxPressed});

  final VoidCallback onAddBoxPressed;
